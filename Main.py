...
def switch_sheet(sheet_name):
    if sheet_name in CSV_FILES:
        ...
        session.pop('filters', None)  # Limpar os filtros atuais
        filters = get_column_filters()  # Recarregar os filtros do novo CSV
...
def apply_filters(df, filter_params):
    ...
    if 'Valor Deferido' in df.columns:
        ...
        if 'Valor Deferido' in filter_params:
            min_value = parse_number(filter_params['Valor Deferido'].get('min'))
            max_value = parse_number(filter_params['Valor Deferido'].get('max'))
            if min_value is not None and max_value is not None:
                df = df[df['Valor Deferido'].notna() & (df['Valor Deferido'] >= min_value) & (df['Valor Deferido'] <= max_value)]
            elif min_value is not None:
                df = df[df['Valor Deferido'].notna() & (df['Valor Deferido'] >= min_value)]
            elif max_value is not None:
                df = df[df['Valor Deferido'].notna() & (df['Valor Deferido'] <= max_value)]

    # Aplica filtros gerais
    for column, filter_value in filter_params.items():
        if column in df.columns and filter_value and column != 'Valor Deferido':
            if isinstance(filter_value, dict):
                if 'min' in filter_value or 'max' in filter_value:
                    if pd.api.types.is_datetime64_any_dtype(df[column]):
                        if filter_value.get('min'):
...
def get_column_filters():
    try:
        csv_path = get_current_csv_path()  # Usar o caminho correto do CSV
        ...
        # Filtro de Valor Deferido
        if 'Valor Deferido' in df.columns:
            df['Valor Deferido'] = df['Valor Deferido'].apply(parse_number)
            df['Valor Deferido'] = pd.to_numeric(df['Valor Deferido'], errors='coerce')
            filters['Valor Deferido'] = {
                'type': 'numeric',
                'min': float(df['Valor Deferido'].min()),
                'max': float(df['Valor Deferido'].max())
            }
